...
public class CustomHelper{
    ...
    private void init(){
        ...
        rgCompressTool= (RadioGroup) rootView.findViewById(R.id.rgCompressTool);//压缩工具
        rgCropSize= (RadioGroup) rootView.findViewById(R.id.rgCropSize);//尺寸/比例：
        rgFrom= (RadioGroup) rootView.findViewById(R.id.rgFrom);//从哪选择：
        rgPickTool= (RadioGroup) rootView.findViewById(R.id.rgPickTool);//使用TakePhoto自带相册：
        rgRawFile = (RadioGroup) rootView.findViewById(R.id.rgRawFile);//拍照压缩后是否保存原图：
        rgCorrectTool= (RadioGroup) rootView.findViewById(R.id.rgCorrectTool);//纠正拍照的照片旋转角度：
        rgShowProgressBar= (RadioGroup) rootView.findViewById(R.id.rgShowProgressBar);//显示压缩进度条：
        rgCropTool= (RadioGroup) rootView.findViewById(R.id.rgCropTool);//裁切工具
        etCropHeight= (EditText) rootView.findViewById(R.id.etCropHeight);//400
        etCropWidth= (EditText) rootView.findViewById(R.id.etCropWidth);//400
        etLimit= (EditText) rootView.findViewById(R.id.etLimit);//选择图片数 这里默认为：1张
        etSize= (EditText) rootView.findViewById(R.id.etSize);//图片大小不超过 102400B
        etHeightPx= (EditText) rootView.findViewById(R.id.etHeightPx);//图片高：800
        etWidthPx= (EditText) rootView.findViewById(R.id.etWidthPx);//图片宽：800
